...
public interface PostJpaRepository extends JpaRepository<PostEntity, Long> {
    ...
    @Query("select new com.example.samplesns.post.dto.DailyPostResponse(p.member.email, p.member.nickname, DATE(p.createDate), count(p.id)) from PostEntity p " +
            "where p.member.id = :memberId " +
            "and DATE(p.createDate) between :firstDate and :lastDate " +
            "group by p.member.email, p.member.nickname, DATE(p.createDate)")
    List<DailyPostResponse> groupByCreateDate(@Param("memberId") long memberId,
